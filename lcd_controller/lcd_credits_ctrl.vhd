...
entity lcd_credits_ctrl is
  port(
    ...
    lcd_busy  : in  std_logic;                          -- ocupado do controlador LCD
    key       : in  std_logic_vector(4 downto 1);       -- não usado, mantido p/ compatibilidade
    credits   : in  unsigned(13 downto 0);              -- 0..9999 do jukebox_credits
    lcd_e     : out std_logic;                          -- enable para o controlador
    lcd_bar   : out std_logic_vector(9 downto 0)        -- (9)rs (8)rw (7..0)dado
  );
end entity;

architecture bhv of lcd_credits_ctrl is
  -- Registradores de interface com o controlador LCD
  signal lcd_enable : std_logic := '0';
  signal lcd_bus    : std_logic_vector(9 downto 0) := (others => '0');

  -- Linhas do LCD (cada uma com 16 chars → 128 bits)
  ...
  -- Pisca/efeitos
  signal blink_cnt : unsigned(23 downto 0) := (others => '0');  -- ajuste de taxa
  ...
  -- Função: converte credits (0..9999) para 4 dígitos ASCII
...
begin
  -- Saídas ao controlador
  lcd_e   <= lcd_enable;
  lcd_bar <= lcd_bus;

  --------------------------------------------------------------------
  -- Geração das linhas L1/L2 a partir de TITLE e dos créditos
  --------------------------------------------------------------------
  process(clk)
  ...
  begin
    if rising_edge(clk) then
      ...
      -- Pisca suave (ajuste blink_cnt para velocidade desejada)
      blink_cnt <= blink_cnt + 1;
      if blink_cnt = x"4F_FFFF" then
        blink_cnt <= (others => '0');
        blink_on  <= not blink_on;
      end if;

      -- Linha 2: com ou sem créditos
      if credits = 0 then
        -- Quando não há créditos, alterna entre "INSERT COIN" e "CREDITS: 0000"
        if blink_on = '1' then
          line2 := INSERT_COIN;
        else
          digs  := to_4dec(credits);
          -- "CREDITS: " (9 chars) + 4 dígitos + 3 espaços = 16
        ...
        -- Com créditos: mostra sempre "CREDITS: ####"
        digs  := to_4dec(credits);
        line2(1)  := 'C'; line2(2)  := 'R'; line2(3)  := 'E'; line2(4)  := 'D';
        line2(5)  := 'I'; line2(6)  := 'T'; line2(7)  := 'S'; line2(8)  := ':';
        line2(9)  := ' ';
      ...
      -- Converte para vetores de 128 bits (cada char = 8 bits)
  ...
  begin
    if rising_edge(clk) then
      if (lcd_busy = '0' and lcd_enable = '0') then
        ...
        case char is
          when 0  => lcd_bus <= "00" & "10000000"; -- set DDRAM linha 1
          ...
          when 17 => lcd_bus <= "00" & "11000000"; -- set DDRAM linha 2
