/**
 * @file    buzzer_sweet.v
 * @brief   Módulo de topo para tocar "Sweet Child O' Mine".
...
module buzzer_sweet (
    ...
    //-----------------------------------------------------------------------------
    // run_en: "chave geral" que controla se a música está tocando/avançando.
    reg run_en;
    // mute_en: controla se o som está mudo.
    reg mute_en;
    // is_stopped: diferencia entre uma parada total e uma pausa.
    reg is_stopped;

    // Registradores para amostragem dos botões para detecção de borda
    reg play_d1, play_d2;
    reg stop_d1, stop_d2;
    reg mute_d1, mute_d2;
    // stop_pulse: pulso interno de 1 ciclo para resetar o controlador da melodia.
    reg stop_pulse;
    ...
    initial begin
        run_en     = 1'b0;
        mute_en    = 1'b1; // Inicia mudo (ou 1'b0 para iniciar com som)
        is_stopped = 1'b1;
        start_play = 1'b0;
        play_d1 = 0; play_d2 = 0;
        stop_d1 = 0; stop_d2 = 0;
    ...
    //-----------------------------------------------------------------------------
    controlador_sweet u_ctrl (
        .Clk_in   (clock),
        .Play_in  (run_en),       // A música avança se run_en for 1
        .Stop_in  (stop_pulse),   // Pulso de reset para o controlador
        .Duracao  (duracao),
        .Disparo  (disparo),
        .Temp_out (temp_ovf),
        .Freq_out (freq_ovf),
    ...
    //-----------------------------------------------------------------------------
    // O som do buzzer só é ativado se a música estiver tocando (run_en) e não estiver mudo
    assign buzzer   = (run_en && !mute_en) ? tone : 1'b0;
    // A saída song_end é simplesmente o fio interno conectado à porta de saída
